...
def launch_pair_viewer(args: PairInputs) -> None:  # pragma: no cover - GUI
    ...
    disp_row.addWidget(QtWidgets.QLabel("Display:")); disp_row.addWidget(disp_mode)
    center_chk = QtWidgets.QCheckBox("Center at chem (bottom)")
    center_chk.setChecked(False)
    disp_row.addWidget(center_chk)
    ...
    hp_order_spin = QtWidgets.QSpinBox(); hp_order_spin.setRange(2, 6); hp_order_spin.setValue(4)
    ...
    bp_order_spin = QtWidgets.QSpinBox(); bp_order_spin.setRange(2, 6); bp_order_spin.setValue(4)
    detrend_combo = QtWidgets.QComboBox(); detrend_combo.addItems(["Median", "Savitzky–Golay"]) 
    detrend_win_spin = QtWidgets.QDoubleSpinBox(); detrend_win_spin.setRange(0.020, 0.100); detrend_win_spin.setDecimals(3); detrend_win_spin.setValue(0.050); detrend_win_spin.setSuffix(" s")
    ...
    # Build vertical panel of controls
    ...
    win_form.addRow("Chem window:", chem_chk)
    ...
    filt_form.addRow("Mode:", filt_mode)
    filt_form.addRow("HP cutoff:", hp_spin)
    filt_form.addRow("HP order:", hp_order_spin)
    filt_form.addRow("BP low:", bp_lo_spin)
    filt_form.addRow("BP high:", bp_hi_spin)
    filt_form.addRow("BP order:", bp_order_spin)
    filt_form.addRow("Detrend:", detrend_combo)
    ...
    def _set_filter_controls_visibility() -> None:
        ...
        try:
            hp_order_spin.setVisible(mode in ("High-pass", "Detrend+HP"))
        except Exception:
            pass
        bp_lo_spin.setVisible(mode == "Band-pass")
        bp_hi_spin.setVisible(mode == "Band-pass")
        try:
            bp_order_spin.setVisible(mode == "Band-pass")
        except Exception:
            pass
